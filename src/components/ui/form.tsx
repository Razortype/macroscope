...
const useFormField = () => {
  ...
  if (!fieldContext.name) throw new Error("useFormField must be inside <FormField>");
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <Label
      ref={ref}
      htmlFor={id}
      className={cn(error && "text-severity-high-fg", className)}
      {...props}
    />
  );
});
...
>(({ className, children, ...props }, ref) => {
  ...
  if (!body) return null;
  return (
    <p ref={ref} className={cn("text-xs font-medium text-severity-high-fg", className)} {...props}>
      {body}
    </p>
  );
});
